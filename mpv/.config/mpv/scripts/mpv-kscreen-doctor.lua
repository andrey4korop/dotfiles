...
local MODESFILE = "/tmp/mpv-screen-doctor.modes"
...
local function best_fit(target, options)
	...
	--[[for _, mode in pairs(options) do
		local offset = math.fmod(mode.rate, target)/target
		msg.info("offset " .. offset .. " mode.rate " .. mode.rate)
		if offset < best.distance then
			best = {
				distance = offset,
				id = mode.id,
...
local function set48()
	--local is_pause = mp.get_property_bool('pause')
	--mp.set_property_bool('pause', true)
    local modes = {}
    modes[1] = 2
    kscreen_doctor_set_mode(modes)
	mp.osd_message('Set 48fps mode', 3)
	--if is_pause==false then
	--	mp.set_property_bool('pause', false)
	--end
end

local function set60()
	--local is_pause = mp.get_property_bool('pause')
	--mp.set_property_bool('pause', true)
    local modes = {}
    modes[1] = 1
    kscreen_doctor_set_mode(modes)
	mp.osd_message('Set 60fps mode', 3)
	--if is_pause==false then
	--	mp.set_property_bool('pause', false)
	--end
end
...
if r.status == 0 then
	...
	PIDFILE = "/run/user/" .. uid .. "/mpv-kscreen-doctor.pid"
else
...
mp.add_key_binding(nil, 'set48', set48)
mp.add_key_binding(nil, 'set60', set60)
